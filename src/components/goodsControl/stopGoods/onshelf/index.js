...
import { Button, Cascader, InputNumber, Drawer, Icon, Input, message, Modal, BackTop, Table, Radio } from 'antd';
...
class Onshelf extends Component {
    ...
    componentDidMount() {
        setTimeout(() => {
            ...
            this.props.dispatch(actionCreators.changeBreadcrumb(['商品管理', '已下架商品', '上架商品']));
            let data = this.props.location.state;
            if (data !== undefined) {
                const sku = data.record.sysSku;
                if (sku) {
                    const jdDefaultCategory = this.handleFormatTypeValue([data.record.categoryObj]);
                    this.getGoodsDetail(sku);
                    this.setState({jdOptions: data.optionList});
    ...
    handleSaveClick = () => {
        ...
        const url = `${baseURL}goods/details/onshelf`;
                        ...
                        <Container style={{width: '57%'}}>
                            ...
                            <ChannelView>
                                ...
                                <Options>
                                    ...
                                    <Button type='primary' onClick={this.handleSaveClick}>上架</Button>
                                </Options>
                            </ChannelView>
                        </Container>
                        <Drawer
...
export default connect(null, null)(Onshelf);