...
import { View, Item, Title, Info, OptionWarp } from './style.js';
...
class CreateCoupon extends Component {
    ...
    componentWillMount() {
        this.getTypeList();
        this.props.dispatch(actionCreators.changeBreadcrumb(['运营管理', '优惠券管理', '新增优惠券']));
    }
    /*****************************获取分类列表************************/
    getTypeList() {
        ...
        fatch(url, 'post', {}, (err, state) => {
            ...
            if (res.code === '0') this.setState({ treeData: this.handleFormatTypeList(res.data) });
            else message.error('获取分类id失败：' + res.msg);
        });
    }
    /*****************************格式化分类数据**********************/
    ...
    handleSaveClick = () => {
        ...
        if (params.type === 0) {
            if (params.validateTimeStart === '') {
                message.info('优惠券开始有效期不能为空。');
                return;
            }
            if (params.validateTimeEnd === '') {
    ...
    render() {
        ...
        return (
            <div>
                            ...
                            <Item>
                                ...
                                <Info id='range'>
                                    <Select
                                        ...
                                        style={{ width: '95%' }}
                                        value={applyType}
                                        onChange={(value) => {this.setState({applyType: value})}}
                ...
                <OptionWarp>
                    <Button type='primary' onClick={() => this.handleBackClick(false)}>取消</Button>
